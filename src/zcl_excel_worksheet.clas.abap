...
*"* do not include other source files here!!!
  ...
  data PRINT_TITLE_ROW_FROM2 type ZEXCEL_CELL_ROW .
  data PRINT_TITLE_ROW_TO2 type ZEXCEL_CELL_ROW .
  ...
  methods GET_COLUMNS_ITERATOR
    ...
    raising
      resumable(CX_SY_REF_IS_INITIAL) .
...
* Do not check merged cells
        ...
        me->get_column_dimension( <auto_size>-col_index )->set_width( <auto_size>-width ).
      ENDLOOP.
    ENDIF.
...
* Return guid if requested.  Might be used if copy&paste of styles is requested
  ...
  METHOD constructor.
    ...
    CREATE OBJECT columns. "Added by Balaji-S
    CREATE OBJECT rows. "Added by Balaji-S
...
* Now build sheetnumber.  Increase counter until we hit a number that is not used so far
  ...
  METHOD get_column.
    ...
    CREATE OBJECT lo_columns.


    lo_column_iterator = lo_columns->get_iterator(  ).
*  lo_column_iterator =  me->columns->get_iterator(  ).
*  lo_column_iterator =  me->columns->get_iterator(  ).
*  lo_column_iterator = me->get_columns_iterator( ).
    WHILE lo_column_iterator->has_next( ) = abap_true.
      lo_column ?= lo_column_iterator->get_next( ).
  ...
  METHOD get_columns_iterator.
    ...
    DATA: eo_columns TYPE REF TO zcl_excel_columns.

*TRY .
*
**  EO_columns = me->columns.
**  eo_iterator =  EO_columns->get_iterator( ).
*   eo_iterator = me->columns->get_iterator( ).
*CATCH cx_sy_ref_is_initial INTO DATA(lr_exc).
...
*ENDTRY.
  ...
  METHOD get_column_dimension.
    ...
    READ TABLE me->column_dimensions ASSIGNING <fs_column_dimension>
      WITH KEY column = lv_column_alpha.
    ...
    IF NOT <fs_column_dimension> IS ASSIGNED.
      ...
      <fs_column_dimension>-column = lv_column_alpha.
...
*--------------------------------------------------------------------*
    ...
    DATA: lo_range_iterator         TYPE REF TO cl_object_collection_iterator,
          ...
          lo_range2                  TYPE REF TO zcl_excel_range,
          ...
          lv_repeat_range_col2       TYPE string,
          lv_row_char_from2          TYPE char10,
          lv_row_char_to2            TYPE char10,
          lv_repeat_range_row2       TYPE string,
          lv_repeat_range2           TYPE string.
...
*--------------------------------------------------------------------*
    ...
    WHILE lo_range_iterator->has_next( ) = abap_true.
      ...
      lo_range2 ?= lo_range_iterator->get_next( ).
      IF lo_range2->name = zif_excel_sheet_printsettings=>gcv_print_title_name.
        EXIT.  " Found it
      ENDIF.
      CLEAR lo_range2.
...
*--------------------------------------------------------------------*
      ...
      ENDIF.
      IF lo_range2 IS BOUND.
        me->ranges->remove( lo_range2 ).
      ENDIF.
    ELSE.
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
      ...
      IF lo_range2 IS NOT BOUND.
        lo_range2 =  me->add_new_range( ).
        lo_range2->name = zif_excel_sheet_printsettings=>gcv_print_title_name.
      ENDIF.
...
*--------------------------------------------------------------------*
      ...
      IF me->print_title_row_from2 IS NOT INITIAL.
        lv_row_char_from2 = me->print_title_row_from2.
        lv_row_char_to2   = me->print_title_row_to2.
        CONCATENATE '!$' lv_row_char_from2
                    ':$' lv_row_char_to2
            INTO lv_repeat_range_row2.
        CONDENSE lv_repeat_range_row2 NO-GAPS.
        CONCATENATE lv_repeat_range_sheetname
                    lv_repeat_range_row2
            INTO lv_repeat_range_row2.
      ENDIF.
...
*      lo_range->set_range_value( lv_repeat_range ).
        ...
        lv_repeat_range2 = lv_repeat_range_row2.

*        CONCATENATE lv_repeat_range_col2 lv_repeat_range_row2
*            INTO lv_repeat_range2 SEPARATED BY ','.
*      ENDIF.


      lo_range->set_range_value( lv_repeat_range ).
      lo_range2->set_range_value( lv_repeat_range2 ).
    ENDIF.
...
*    me->print_title_row_from2 = iv_rows_from.
*    me->print_title_row_to2   = iv_rows_to.
